...
export const lyricsApi = {
  ...
  create(payload: {
    ...
    return call<{ template: LyricTemplate }>("kanvas-lyrics-template", { action: "create", ...payload });
  },
  get(templateId: string) {
    return call<{ template: LyricTemplate }>("kanvas-lyrics-template", { action: "get", templateId });
  },
  list() {
    return call<{ templates: LyricTemplate[] }>("kanvas-lyrics-template", { action: "list" });
  },
  patch(templateId: string, patch: Partial<{
    title: string;
  ...
  finalize(templateId: string) {
    return call<{ template: LyricTemplate }>("kanvas-lyrics-template", { action: "finalize", templateId });
  },
  archive(templateId: string) {
    return call<{ template: LyricTemplate }>("kanvas-lyrics-template", { action: "archive", templateId });
  },
