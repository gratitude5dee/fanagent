...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const item = await supabase
      ...
      .select(
        "id,account_id,batch_id,stock_clip_url,input_payload,duration_seconds,lyric_template_id,metadata",
      )
      .eq("id", body.itemId)
      .single();
    ...
    const lyricFont = pickFont(body.itemId);
    ...
    if (lyricTemplateId) {
      ...
      if (blocks.length > 0) {
        ...
        finalUrl = await composeWithSubtitles({
          ...
          totalSeconds,
          fontFamily: lyricFont.name,
          fontWeight: lyricFont.weight,
          outlineColor: lyricFont.outline,
        });
        provider = "fal_ffmpeg";
      }
    }
    ...
    const asset = await createMediaAssetFromBytes({
      ...
      metadata: {
        ...
        lyric_font: lyricFont.name,
        lyric_font_weight: lyricFont.weight,
      },
    });
